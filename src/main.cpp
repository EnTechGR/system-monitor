...
void memoryProcessesWindowContent()
{
...
void advancedMetricsWindowContent()
{
...
int main(int, char **)
{
    ...
    SDL_GL_SetSwapInterval(0); // Disable VSync to prevent the driver from spinning/polling
    ...
    while (!done)
    {
...

        {
            ...
            // Smart Redraw: Only process window logic if visible
            if (ImGui::Begin("== System ==", nullptr, ImGuiWindowFlags_None)) {
                ImGui::SetWindowSize(ImVec2(midX - pad * 1.5f, midY - pad * 1.5f));
                ImGui::SetWindowPos(ImVec2(pad, pad));
                systemWindowContent();
            }
            ImGui::End();

            if (ImGui::Begin("== Memory and Processes ==", nullptr, ImGuiWindowFlags_None)) {
                ImGui::SetWindowSize(ImVec2(midX - pad * 1.5f, midY - pad * 1.5f));
                ImGui::SetWindowPos(ImVec2(midX + pad / 2.0f, pad));
                memoryProcessesWindowContent();
            }
            ImGui::End();

            if (ImGui::Begin("== Network ==", nullptr, ImGuiWindowFlags_None)) {
                ImGui::SetWindowSize(ImVec2(midX - pad * 1.5f, midY - pad * 1.5f));
                ImGui::SetWindowPos(ImVec2(pad, midY + pad / 2.0f));
                networkWindowContent();
            }
            ImGui::End();

            if (ImGui::Begin("== Advanced Metrics ==", nullptr, ImGuiWindowFlags_None)) {
                ImGui::SetWindowSize(ImVec2(midX - pad * 1.5f, midY - pad * 1.5f));
                ImGui::SetWindowPos(ImVec2(midX + pad / 2.0f, midY + pad / 2.0f));
                advancedMetricsWindowContent();
            }
            ImGui::End();
        }
        ...
        // Ultra-efficiency Mode: 15 FPS (66ms delay).
        // For a system monitor, this is extremely lean while remaining readable.
        SDL_Delay(66); 
    }
